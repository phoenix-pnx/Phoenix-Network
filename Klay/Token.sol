...
contract BasicToken is ERC20Basic {
    ...
    function transfer(address _to, uint256 _value) public returns (bool) {
        require(
            _to != address(0),
            "Recipient address is zero address(0). Check the address again."
        );
        require(
...
contract StandardToken is ERC20, BasicToken {
    ...
    function transferFrom(
        ...
        require(
            ...
            "Insufficient tokens approved from account owner."
        );
...
contract MultiOwnable {
     ...
     */
    function changeRoot(address newRoot) public onlyRoot returns (bool) {
        require(
            newRoot != address(0),
            "This address to be set is zero address(0). Check the input address."
        );
     ...
     */
    function changeSuperOwner(address newSuperOwner)
        public
        onlyRoot
        returns (bool)
    {
        require(
            newSuperOwner != address(0),
            "This address to be set is zero address(0). Check the input address."
        );
    ...
    function newOwner(address owner) public onlySuperOwner returns (bool) {
        require(
            owner != address(0),
            "This address to be set is zero address(0). Check the input address."
        );
...
contract LockableToken is StandardToken, MultiOwnable {
    ...
    function unlockTo(address addr, string note) public onlyOwner {
        if (lockValues[addr] == LOCK_MAX) setLockValue(addr, 0, note);
    ...
    function transferFrom(
        ...
        address to,
        uint256 value
    ) public checkUnlock(from, value) returns (bool) {
...
contract SobloTokenReceiver {
    enum SobloTokenReceiveType {
        TOKEN_TRANSFER,
        TOKEN_MINT
    }

    function onSobloTokenReceived(
        address owner,
        address spender,
        uint256 value,
        SobloTokenReceiveType receiveType
    ) public returns (bool);
}
...
contract SobloToken is LockableToken {
    ...
    function sobloTransfer(
        ...
        require(
            to != address(this),
            "The receive address is the Contact Address of Soblo Token. You cannot send money to this address."
        );
        ...
        postTransfer(
            ...
            value,
            note,
            SobloTokenReceiver.SobloTokenReceiveType.TOKEN_TRANSFER
        );

        emit SobloTransferred(
            msg.sender,
            to,
            value,
    ...
    {
        return
            sobloIncreaseApproval(
                spender,
                addedValue,
                "called by increaseApproval()"
            );
    }

    function sobloIncreaseApproval(
        address spender,
        uint256 addedValue,
        string note
    ) public returns (bool ret) {
    ...
    {
        return
            sobloDecreaseApproval(
                spender,
                subtractedValue,
                "called by decreaseApproval()"
            );
    }

    function sobloDecreaseApproval(
        address spender,
    ...
    function postTransfer(
        ...
        if (to.isContract()) {
            bool callOk = address(to).call(
                bytes4(
                    keccak256(
                        "onSobloTokenReceived(address,address,uint256,uint8)"
                    )
                ),
                owner,
                spender,
    ...
    function sobloMultiTransfer(
        ...
        require(
            ...
            "The size of 'to' and 'values' array is different."
        );
...
contract SobloTokenDappBase is SobloTokenReceiver {
    ...
    modifier onlySobloToken() {
        require(
            msg.sender == _sobloToken,
            "msg.sender must be the registered token contract"
        );
        _;
    }
